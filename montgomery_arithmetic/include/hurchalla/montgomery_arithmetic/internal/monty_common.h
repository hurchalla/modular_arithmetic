...
// R = 2^(std::numeric_limits<T>::digits).  For example, if T is uint64_t, then
// R = 2^64.
...
template <typename T>
FORCE_INLINE T REDC_non_minimized(bool& ovf, T u_hi, T u_lo, T n, T neg_inv_n)
{
    ...
    assert_logic2(mn_hi <= static_cast<T>(0) - static_cast<T>(2));
    ...
    // is always false.
    postcondition2((u_hi == 0) ? ovf == false : true);
    ...
    postcondition2((u_hi == 0 && u_lo < n) ? t_hi < n : true);
    ...
    if (POSTCONDITION2_MACRO_IS_ACTIVE) {
        ...
        postcondition2((n < Rdiv2) ? (ovf == false && t_hi < 2*n) : true);
    }
...
//   R = 2^(std::numeric_limits<T>::digits).
...
//    T2 t = u + mn;
//    ovf = (t < u);
//    T t_hi = static_cast<T>(t >> std::numeric_limits<T>::digits);
//    return t_hi;
// }
...
T typecast_montmul_non_minimized(bool& ovf, T x, T y, T n, T neg_inv_n)
{
    ...
    static_assert(std::numeric_limits<T2>::is_modulo, "");
    ...
    using V2 = typename make_safe_unsigned_integer<T2>::type;
    static_assert(std::numeric_limits<V>::is_modulo, "");
    static_assert(std::numeric_limits<V2>::is_modulo, "");
    ...
    precondition2(u < static_cast<V2>(n) * R);
    ...
    postcondition2((y==1) ? ovf==false : true);
    ...
    // Multiplication Technique: How to Make It Smaller and Faster", at
    // https://www.comodo.com/resources/research/cryptography/CDW_CHES_99.ps ]
    postcondition2((y==1 && x<n) ? t_hi<n : true);
...
template <typename T>
FORCE_INLINE T impl_montmul_non_minimized(bool& ovf, T x, T y, T n, T neg_inv_n)
{
    T u_lo;
    ...
    if (POSTCONDITION2_MACRO_IS_ACTIVE) {
        T Rdiv2 = static_cast<T>(1) << (std::numeric_limits<T>::digits - 1);
        postcondition2((n < Rdiv2) ? (ovf == false && result < 2*n) : true);
    }
...
// For postconditions, see inside this function.
template <typename T>
FORCE_INLINE T impl_montout_non_minimized(T x, T n, T neg_inv_n)
{
    ...
    postcondition2((x < n) ? result < n : true);
...
#if TARGET_BIT_WIDTH >= 16
FORCE_INLINE uint8_t impl_montmul_non_minimized(bool& ovf, uint8_t x, uint8_t y,
                                                   uint8_t n, uint8_t neg_inv_n)
{
...
#if TARGET_BIT_WIDTH >= 32
FORCE_INLINE uint16_t impl_montmul_non_minimized(bool& ovf, uint16_t x,
                                     uint16_t y, uint16_t n, uint16_t neg_inv_n)
{
    using T = uint16_t;
    using T2 = uint32_t;
    T result = typecast_montmul_non_minimized<T, T2>(ovf, x, y, n, neg_inv_n);
...
FORCE_INLINE uint32_t impl_montmul_non_minimized(bool& ovf, uint32_t x,
                                     uint32_t y, uint32_t n, uint32_t neg_inv_n)
{
    using T = uint32_t;
    using T2 = uint64_t;
    T result = typecast_montmul_non_minimized<T, T2>(ovf, x, y, n, neg_inv_n);
...
template <typename T>
FORCE_INLINE T montmul_non_minimized(bool& ovf, T x, T y, T n, T neg_inv_n)
{
    // Precondition: Assuming theoretical unlimited precision standard
    // multiplication, this function requires  x*y < n*R.  (The constant R
    // represents the value R = 2^(std::numeric_limits<T>::digits))
...
// Converts the montgomery value 'x' to a non-minimized (mod n) standard integer
template <typename T>
FORCE_INLINE T montout_non_minimized(T x, T n, T neg_inv_n)
{
    namespace dmc = detail_monty_common;
